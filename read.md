...
用户可以在底部输入框直接输入症状，例如“嗓子痛”。

### 2. 登录或注册

当未登录用户开始问诊时，系统会弹出登录/注册窗口。

弹窗内容包括：

- 标题：登录后继续问诊
- 说明：问诊记录和近期症状记忆会保存到您的账号。
- 登录/注册切换按钮
- 用户名输入框
- 密码输入框
- 登录并继续 / 注册并继续按钮

登录成功后，页面返回问诊界面，右上角显示当前用户名和“退出”按钮。

### 3. 输入主诉

登录后，用户输入主要不适并发送。示例截图中用户输入：

```text
嗓子痛
```

系统会进入处理中状态，显示思考动画，并开始进行意图识别、症状抽取和问诊流程。

### 4. 补充或修正症状

系统会根据用户描述展示可补充的症状维度。示例中系统提示：
...
随后展示“请补充或修正相关症状”的十问维度列表，常见维度包括：
...
系统会记录用户选择的症状，并生成 2 个确认问题。示例中系统回复：
...
点击“确认答案”后，系统继续整理问诊信息。

### 6. 查看诊断结论

确认完成后，系统生成结构化诊断结论。示例报告包含以下部分：
